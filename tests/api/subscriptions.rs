...
async fn subscribe_sends_a_confirmation_email_with_a_link() {
    ...
    // Path is specific to mailtrap, book uses another email provider
    Mock::given(path_regex(r"^/api/send/\d+"))
        .and(method("POST"))
        .respond_with(ResponseTemplate::new(200))
        .mount(&app.email_server)
    ...
    // Leverage new helper for simpler conf link access
...
async fn subscribe_returns_a_200_for_valid_form_data() {
    ...
    // This path is for mailtrap, book uses another email provider
    Mock::given(path_regex(r"^/api/send/\d+"))
        .and(method("POST"))
        .respond_with(ResponseTemplate::new(200))
        .mount(&app.email_server)
    ...
    let responose = app.post_subscriptions(body.into()).await;

    assert_eq!(200, responose.status().as_u16());
}
